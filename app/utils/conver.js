...
String.prototype.tran = function () {
  ...
  const zhChoose = uni.getStorageSync('lang')
...
// 暴露出去，执行函数，uniapp可用， 每个页面都要调用，需要把页面中要转换的文字存在data里

export const zhTran = function (obj) {
  const o = obj
    ...
    if (typeof o[0] == 'string') {
      ...
      for (let i = 0; i < o.length; i++) {
        for (let u in o[i]) {
          ...
          if (typeof o[i][u] != 'number') {
            k[u] = String(o[i][u]).tran()
          } else {
            //如果是数字不进行转换

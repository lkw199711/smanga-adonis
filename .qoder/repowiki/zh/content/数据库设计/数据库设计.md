...
## 更新摘要
**变更内容**
- 更新了manga表的字段定义，将chapterCount从可空改为非空，默认值为0
- 新增了chapterUpdate字段到manga表，所有数据库类型均支持此字段
- 更新了各数据库的时间戳精度说明，反映了MySQL和PostgreSQL的差异
- 更新了实体关系图和字段定义表格，反映最新的数据库结构
...
**更新** 新增了chapterUpdate字段到manga表，所有数据库类型均支持此字段。MySQL使用DATETIME(6)精度，PostgreSQL使用TIMESTAMP(3)精度。
...
- **章节计数约束**：manga表的chapterCount字段已从可空改为非空，默认值为0。

**更新** manga表的chapterCount字段现在是非空的，确保所有漫画都有明确的章节计数。
...
**更新** MySQL使用DATETIME(6)微秒精度，PostgreSQL使用TIMESTAMP(3)毫秒精度，提高了时间戳的精确度。
...
- **迁移路径**
  ...
  - **最新变更**：2026年4月2日新增chapterUpdate字段到manga表，并将chapterCount设为非空默认值0。
- **版本管理**
  - 版本表用于记录系统版本号与描述，便于追踪升级状态。
  - 建议每次数据库变更均新增迁移文件，并在生产环境执行前进行回滚测试。

**更新** 添加了2026年4月2日的数据库变更迁移文件，反映了manga表的重要结构改进。

**章节来源**
- [prisma/mysql/migrations/20240817084208_init/migration.sql:1-449](file://prisma/mysql/migrations/20240817084208_init/migration.sql#L1-L449)
- [prisma/pgsql/migrations/20240817084740_init/migration.sql:1-479](file://prisma/pgsql/migrations/20240817084740_init/migration.sql#L1-L479)
- [prisma/sqlite/migrations/20240817081809_init/migration.sql:1-385](file://prisma/sqlite/migrations/20240817081809_init/migration.sql#L1-L385)
- [prisma/mysql/migrations/20260402181735_chapter_update/migration.sql:1-10](file://prisma/mysql/migrations/20260402181735_chapter_update/migration.sql#L1-L10)
- [prisma/pgsql/migrations/20260402181850_chapter_update/migration.sql:1-11](file://prisma/pgsql/migrations/20260402181850_chapter_update/migration.sql#L1-L11)
- [prisma/sqlite/migrations/20260402181920_chapter_update/migration.sql:1-35](file://prisma/sqlite/migrations/20260402181920_chapter_update/migration.sql#L1-L35)
...
- **差异点**
  ...
  - **MySQL**：支持 UNSIGNED 整型、更丰富的字符串类型与 JSON 原生类型；时间戳支持微秒级精度（DATETIME(6)）。
  - **PostgreSQL**：支持 JSONB 原生类型、更严格的类型系统与毫秒级时间戳（TIMESTAMP(3)）。
- **配置方式**
  - 运行时根据配置选择数据库类型与连接参数；SQLite 支持 Windows 与类 Unix 路径差异。
  - 示例配置文件提供 SQLite 默认路径与参数示例。

**更新** MySQL和PostgreSQL现在支持不同的时间戳精度，MySQL使用DATETIME(6)，PostgreSQL使用TIMESTAMP(3)。
...
- **时间戳精度**
  - MySQL使用DATETIME(6)微秒精度，PostgreSQL使用TIMESTAMP(3)毫秒精度，提高时间戳的精确度。
...
- **迁移异常**
  ...
  - **特别注意**：2026年4月2日的迁移可能因chapterCount字段的非空约束而失败，需要先处理现有NULL值。
- **性能问题**
  - 分析慢查询日志，补充缺失索引或调整查询策略。

**更新** 新增了关于2026年4月2日迁移的故障排查指导，特别关注chapterCount字段的非空约束。
...
- [prisma/pgsql/migrations/20240817084740_init/migration.sql:408-479](file://prisma/pgsql/migrations/20240817084740_init/migration.sql#L408-L479)
- [prisma/sqlite/migrations/20240817081809_init/migration.sql:1-385](file://prisma/sqlite/migrations/20240817081809_init/migration.sql#L1-L385)
- [prisma/mysql/migrations/20260402181735_chapter_update/migration.sql:4](file://prisma/mysql/migrations/20260402181735_chapter_update/migration.sql#L4)

## 结论
本设计通过统一的 Prisma 模型与多数据库迁移脚本，在 SQLite、MySQL、PostgreSQL 之间实现了高度一致的数据结构与约束。结合运行时配置与连接参数，应用可在不同环境中无缝切换。

**重要更新**：2026年4月2日的数据库变更显著改进了manga表的结构，将chapterCount字段设为非空默认值0，并为所有数据库类型添加了chapterUpdate字段。这些变更确保了数据完整性并提供了更好的章节更新跟踪能力。

建议在生产环境中完善缓存、监控与归档策略，并持续维护迁移脚本以保障数据演进的可控性与可追溯性。
...
- **数据库变更历史**
  - **2024年8月17日**：初始数据库结构
  - **2026年4月2日**：manga表重大改进（chapterCount非空化，新增chapterUpdate字段）
